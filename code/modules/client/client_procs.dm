...
*/
	...
	if(!client_is_in_db)
		...
		if (CONFIG_GET(flag/panic_bunker) && !holder && !GLOB.deadmins[ckey] && !(ckey in GLOB.bunker_passthrough))
			...
			to_chat_immediate(src, {"<span class='notice'>Bem-vindo ao Estação Nebula 13. Por questões de segurança, este servidor tem um sistema de whitelist (Panic Bunker) que impede novos jogadores de entrarem sem se aplicarem no Discord do servidor.<br>Siga as instruções abaixo e ABRA UM TICKET DE SUPORTE através do canal #suporte em nosso Discord. <br> <br> Para que você seja adicionado à whitelist, você precisa acessar o nosso servidor de Discord (<a href='https://discord.gg/bFEHtJ7KAp'>https://discord.gg/bFEHtJ7KAp</a>) e, logo depois, dirigir-se ao canal #suporte, abrir um ticket e solicitar acesso ao servidor.</span>"}) //nebula welcome message
			var/list/connectiontopic_a = params2list(connectiontopic)
			var/list/panic_addr = CONFIG_GET(string/panic_server_address)
			if(panic_addr && !connectiontopic_a["redirect"])
				var/panic_name = CONFIG_GET(string/panic_server_name)
